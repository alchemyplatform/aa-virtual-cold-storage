...
interface IColdStoragePlugin {
    ...
    event ERC721Locked(address indexed account, uint256 duration);
    ...
    /// @notice Add a storage key to the sender.
    /// @param storageKey The storage key to add.
    function addStorageKey(address storageKey) external;

    /// @notice Replace the sender's storage key with a new one.
    /// @param storageKey The account to replace.
    function replaceStorageKey(address storageKey) external;
    ...
    function lockERC721All(uint256 duration) external;
